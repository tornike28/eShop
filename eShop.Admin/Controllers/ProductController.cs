...
namespace eShop.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public string GetProductInfo(string id)
        {
            ...
            var product = _ProductApplicationService.AdminGetProduct(productId).FirstOrDefault();
            return JsonSerializer.Serialize(product);
        }

        public async Task<IActionResult> SaveProduct(UpdateProductModel productModel)
        {
            List<string> fileNames = null;

            if (productModel.ImageFiles is not null)
            {
                 fileNames = await _ProductApplicationService.UploadImages(productModel.ImageFiles);
            }

            //Insert record

            var query = _ProductApplicationService.SaveProduct(new ProductDTO()
            {
                Id = productModel.Id,
